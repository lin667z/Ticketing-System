...
public class AgentDispatcher {
    ...
    private static final int DEFAULT_TASK_TIMEOUT_MS = 10000;
    ...
    public Mono<List<AgentTaskResult>> dispatch(AgentPlan plan, AgentTraceEmitter traceEmitter) {
        if (plan == null || plan.getTasks() == null || plan.getTasks().isEmpty()) {
            return Mono.just(List.of());
        }
        if (traceEmitter != null) {
            traceEmitter.emitStatus("DISPATCH", "任务调度", "准备执行 Agent 任务：" + plan.getTasks().size() + " 个");
        }
    ...
    private Mono<AgentTaskResult> dispatchOne(AgentTask task, AgentTraceEmitter traceEmitter, boolean streamToUser) {
        ...
        if (traceEmitter != null) {
            traceEmitter.emitStatus(task.getType(), "任务已分配，正在执行");
        }
